...
(context window), long-term (vector stores), and the cognitive architectures that organize them.
...
## Query Routing Decision Framework (2026-06-27)

> **D2 Loop Lesson**: Gap identified 2026-06-23 in `agentic-rag-upgrade-20260623.md` but SKILL.md never patched.
> This section is the D3 exit. Patch the existing skill, don't make a new file.

### When to Use Which Retrieval Strategy

The skill has full theory on chunking and vector stores, but no explicit
**query routing** — deciding *which retrieval strategy* to use at query time.
Without this, agents default to pure semantic (dense) for everything, which
is wrong for ~40% of real queries.

**Three retrieval modes:**

| Mode | When to use | Why |
|------|-------------|-----|
| **Dense (vector semantic)** | Abstract concepts, synonyms, paraphrases ("similar X", "like that thing") | Embedding captures meaning, not keywords |
| **Sparse (BM25 / keyword)** | Exact terms, proper nouns, dates, IDs, technical jargon | Lexical matching beats semantic for precise facts |
| **Hybrid (RRF merge)** | Mixed signals, ambiguous queries, high-stakes answers | Combines strengths, reduces failure modes |

### Query Classification Signal Table

At query time, examine the query for these signals — they override each other
in priority order (top = highest priority):

| Signal pattern | Example | Strategy |
|---------------|---------|----------|
| Exact entity name | "師大附中", "Article 12-3", "user@example.com" | Sparse (BM25) |
| Date / number range | "2026年6月", "first half of 2025" | Sparse + metadata filter |
| Comparison ("X vs Y", "difference between") | "compare the two candidates" | Hybrid + decomposition |
| Aggregation ("count", "list all", "total") | "how many applicants from 師大附中" | Sparse → count |
| Abstract similarity | "similar contracts", "like the previous case" | Dense |
| "Why" / "explain" | "why was this rejected" | Dense + generative |
| Multi-hop | "who approved the candidate from school X" | Decompose → multi-step |

**If** query has ≥2 strong signals pointing to different strategies
**Then** use Hybrid (RRF) as default

### Query Decomposition for Multi-Hop Questions

**If** question involves multiple relationships or steps
**Then** decompose before retrieving:

```
Question: "這個月的面試候選人有多少來自師大附中？"
  ├─ Sub-q1: "師大附中 2026面試候選人"
  ├─ Sub-q2: "2026年6月 面試記錄"
  └─ Sub-q3: "各高中 候選人來源統計"
```

Pattern: break into atomic lookups → merge results → answer.

### Hybrid Retrieval with RRF

```python
# Step 1: Run dense (vector) query
dense_results = chroma.query(query_embedding, top_k=20)

# Step 2: Run sparse (BM25) query
sparse_results = bm25_index.search(query, top_k=20)

# Step 3: RRF merge (Reciprocal Rank Fusion)
# RRF score = sum(1/(k+rank)), k=60 is standard
def rrf_merge(dense_results, sparse_results, k=60):
    merged = {}
    for rank, doc_id in enumerate(dense_results['ids'][0]):
        merged[doc_id] = merged.get(doc_id, 0) + 1/(k + rank + 1)
    for rank, doc_id in enumerate(sparse_results['ids']):
        merged[doc_id] = merged.get(doc_id, 0) + 1/(k + rank + 1)
    return sorted(merged, key=merged.get, reverse=True)
```

**Hermes status**: `rank_bm25` is available (confirmed 2026-06-23). ChromaDB available.
No cross-encoder reranker — use RRF for Hermes's RAG stack.

### Agentic Retrieval: The 2026 Paradigm Shift

> **New (2026-07-10):** Traditional RAG is being displaced by agent-as-retriever in coding agents.

**Key data points:**
- Claude Code removed vector search → outperformed RAG "by a lot" (Boris Cherny)
- Anthropic multi-agent: **90.2% lift** over single Opus 4
...
**When to prefer agentic retrieval over vector search:**
- Codebase navigation, dependency tracing, architecture analysis
- Tasks requiring multi-step reasoning across files
- Queries where filesystem state is the source of truth (not documents)
- Situations where index freshness matters (file edited → agent sees it in 100ms)

**When to keep vector search:**
- Unstructured document collections (policies, guidelines, past reports)
- Semantic similarity across large corpora
- User preference / profile memory
- Scenarios where grep/glob can't express the query

### If→Then Rules (Query Routing)

- **If** query has specific entity names or dates → Sparse (BM25), not dense
- **If** pure semantic similarity needed → Dense only
- **If** query is code/architecture/tracing → Agentic retrieval (Glob/Grep/Read) preferred over vector
- **If** query ambiguous or high-stakes → Hybrid (RRF)
- **If** first retrieval result score < 0.3 → Switch strategy (dense→sparse or agentic), don't re-run same failing strategy
- **If** multi-hop question → Decompose first, then merge
...
Works well with: `autonomous-ai-agents`, `multi-agent-orchestration`, `llm-architect`, `agent-tool-builder`

### Support Files
- Two-layer search: memory_search → mempalace_search
- `references/agentic-rag-hermes-state-20260627.md` — Hermes RAG stack capability, Python env pitfalls, Naive RAG vs Production RAG benchmarks, query routing quick reference, upgrade path (D2 loop lesson embedded)
- `references/agentic-retriever-paradigm-20260710.md` — 2026 research: agent-as-retriever displacing traditional RAG, Anthropic Claude Code data, AAAI 2026 / Amazon Science benchmarks (94.5% faithfulness), four architecture flavors, Hermes upgrade path
- `references/mempalace-hermes-status-20260711.md` — MemPalace (276 drawers) × Hermes state.db topology, Phase 1 search priority clarification (MemPalace primary, session_search secondary)

## Session Signal: Iteration Budget Reached

When running multi-step memory search (session_search → mempalace fallback), the combined tool calls can hit Hermes's `max_turns: 90` limit mid-response, returning:

```
⚠️ Iteration budget reached (90/90) — response may be incomplete
```

**Prevention**: Combine session_search + mempalace fallback into a single execute_code script with batched tool calls, rather than calling each separately in the main turn. This avoids counting each search as a separate turn.

**Recovery**: User sends follow-up; agent resumes from last meaningful result.
