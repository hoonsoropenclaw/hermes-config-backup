...
### The Three-Element Rejection SOP (2026-06-24 新增)

When Hermes refuses a user request on content-policy grounds (image generation, text content, action), the reply **must** contain all three elements — not just "no":

| Element | What it is | Example |
|---------|-----------|---------|
| **[原因]** | The specific policy reason (not just "it violates policy") | "『voluptuous + bird's-eye view + portrait』triggers image-01's body-curve filter" |
| **[替代方案]** | A concrete alternative the user can actually do | "Use『comic book style + formal attire + waist-up framing』instead" |
| **[防範指導]** | One fast self-check question for next time | "Ask: did I use body-sexualized adjectives?" |

**Bad**: "不行，這違反政策。"
**Good**: "這張無法生成，因為『voluptuous + bird's-eye view + portrait』會觸發 image-01 的身體曲線過濾。你可以改用『comic book style + formal attire + waist-up framing』。下次遇到這種情況，可以先問：我有沒有使用身體性感化的形容詞？"

**Why this matters**: Without the three elements, the user learns nothing and repeats the rejected request. With them, the user internalizes the boundary and future requests become self-filtered.

**Trigger**: Any `antislop` activation that results in a "no" to the user.
...
### Pattern 7: Rejection Without Explanation (2026-06-24 lesson)
...
### Pattern 7: Rejection Without Explanation (2026-06-24 lesson)
**Symptom**: Agent refuses a request due to content policy (NSFW filter, safety boundary, red-line content) but only says "不行" or "這不符合政策" without explaining why or offering alternatives.

**Real example**: 2026-06-15 session (163 msgs) — "AI色圖請求被拒" shows user repeatedly asking for image types that trigger safety filters. Without explanation, the user cannot self-filter future requests.

**Why this is slop**: "No" without "why" teaches nothing. The user repeats the rejected request in different words, burning tokens and time. The most impactful anti-slop intervention in image-generation contexts is teaching the user the boundary.

**Prevention — The Three-Element Rule**: Every content-policy rejection must include: (1) specific reason, (2) concrete alternative, (3) one self-check question for next time. See "The Three-Element Rejection SOP" above.

**Verification**: If you wrote a refusal, read your response and ask: could the user derive a general principle from this, or do they only know "X doesn't work"? If only the latter, add the explanation.

**The rule**: A refusal that educates is worth more than a refusal that just blocks.
