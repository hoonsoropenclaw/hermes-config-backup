...
> **參考文件**：
> - `references/school-hr-automation.md` — 學校 HR 特殊上下文、代理教師填補流程、與 anthropic-* 技能整合對照表
> - `references/d2-trap-and-gap-analysis-20260711.md` — D2 迴圈陷阱發現（scripts/ 為空）、依賴缺口分析
...
## ⚠️ D2 Trap Warning（2026-07-11）

**現狀問題**：
- SKILL.md 內嵌 Python 代碼（嵌入式），但 `scripts/` 目錄為空——這是 D2 迴圈陷阱
- 引用了 `school-interview-scheduler` 和 `hr-document-workflow`，但這兩個 skill 目錄**不存在**
- `linear-hr-workflow` 即 `linear` skill，其 `scripts/linear_api.py` 存在且功能正常

**D3 Exit 修復**：從 SKILL.md 嵌入式代碼手動建立 `scripts/` 檔案：
```bash
mkdir -p ~/.hermes/skills/productivity/hr-automation-workflow/scripts
# 提取 SKILL.md 中的 ```python 區塊，分別存為：
# scripts/resume_parser.py
# scripts/interview_generator.py
# scripts/onboarding_tracker.py
```

**依賴的未建立 Skill（需先建立）**：
- `school-interview-scheduler`（Google Calendar + Meet 面試預約）
- `hr-document-workflow`（DOCX 錄取文件產出）

## HR 端到端自動化鏈路（已知缺口 2026-07-11）

```
himalaya（收取履歷）
  → linear（候選人追蹤 + W4 面試狀態）→ scripts/linear_api.py ✅
  → [MISSING] school-interview-scheduler（面試預約 + Google Meet 邀請）
  → [MISSING] hr-document-workflow（錄取文件產出）
```

**每個環節對應的 Skill**：
| 環節 | Skill | 狀態 |
|------|-------|------|
| 收取履歷 | `himalaya` | 外部 skill |
| 候選人追蹤 | `linear` | ✅ scripts/linear_api.py 存在 |
| 面試預約 + Meet | `school-interview-scheduler` | ❌ 目錄不存在 |
| 錄取文件產出 | `hr-document-workflow` | ❌ 目錄不存在 |

## 相關技能

- `job-post-builder`：建立招聘套件（職缺描述、面談 guide）
- `smb-onboard`：入職流程模板
- `school-interview-scheduler`：Google Calendar + Meet 面試預約自動化（填補本技能的面試安排缺口）
