...
**⚠️ 額外檢查：spec/需求驗證（2026-06-14 新增）**

E2E 10/10 綠 ≠ spec 對。**學校公告系統 v3→v4**：技術上 E2E 可能全綠，但 visitor 視角需求理解錯誤（v3 = 訪客只看「公開」，v4 = 訪客看全部），導致連續兩個版本重部署。

**每次 deployment 前追加這個檢查**：

```
[ ] 對照 PRD / 規格文件，確認本版本實作的行為與 spec 一致
    - 特別檢查：角色視角（訪客/受眾/處室/sysadmin 各應看到什麼）
    - 特別檢查：AND/OR 篩選邏輯是否符合 spec 定義
    - 特別檢查：權限矩陣（誰能 POST/PATCH/DELETE）是否符合 spec
    - 如 spec 不清楚 → 先問使用者確認，不准直接假設後 deploy
```

**If** spec 定義模糊但我選擇「直接假設」然後 deploy  
**Then** 這是 SOP 偏移，事後必補 spec clarification 到對話並更新對應的 PRD/architecture 文件

**school-bulletin 慘案教訓**：「announcement 預設對外」是使用者預期但從未寫進 PRD，導致 v1/v2/v3 全推翻重做。
...
- **If** E2E 10/10 綠但「使用者預期」與 spec 不符 **Then** 這是 spec 缺口、不是 E2E 缺口、追加 requirements-validation 步驟再 deploy（學校公告系統 v3→v4 就是這個模式）
