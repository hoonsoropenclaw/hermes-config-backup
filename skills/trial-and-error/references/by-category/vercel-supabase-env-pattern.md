...
# 重要: gitSource 必含 org + repo (2026-06-11 v3 deploy 踩坑)
# - 漏 org → 400 bad_request "Invalid request: `gitSource` missing required property `org`"
# - 用 repoId (numeric) 也可以、但要事先查 GitHub API 拿 id
# - 兩種寫法都通、本檔用 org/repo 字串版比較直觀
payload = json.dumps({
    'name': 'school-bulletin',
    'gitSource': {
        'type': 'github',
        'org': 'hoonsoropenclaw',     # 必填、漏了 400
        'repo': 'school-bulletin',     # 必填
        'ref': 'main',
    },
    'target': 'production'
})
...
7. **gitSource 必填 `org` + `repo`**（不是 `repoId`） — 漏了 400、就算你以為 Vercel 會自動推、其實會
