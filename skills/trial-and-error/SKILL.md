...
- **If** `hermes cron edit --script` 對 `no_agent=True` 的 job **Then** 將 args 寫入 `prompt` 而非 `script` 欄位，造成 Scheduler 把「script + args」當成 script path 執行失敗。**解法**：直接改 jobs.json，`prompt` 設為 `null`，`script` 只留檔名，`no_agent` 保持 `true`
...
- **If** cron job 的 last_error 顯示執行了「另一個 script」但 jobs.json script 是對的 **Then** 檢查 prompt 是否被汙染（`'hermes-config-backup-daily: hermes-backup-v4.sh --tier1'` 格式）、設為 null
- **If** jobs.json timeout_seconds 設 3600 但仍 timeout at 600s **Then** 可能是 Scheduler 預設上限、需要改 config.yaml 或重啟 gateway
- **If** coverage check 報「'.skills_prompt_snapshot.json' 本機有但 v4 ROOT_SINGLE_FILES 沒列」**Then** 先 `ls ~/.hermes/.skills_prompt_snapshot.json` 確認是不是隱藏檔（在 `~/.hermes/` 而非 hermes 根目錄）
...
- `hermes-internal` — hermes CLI / gateway / cron jobs / config / skill 機制的坑（**2026-06-11 新增**：jobs.json prompt/context_from 欄位污染修復 SOP + coverage check 雙檔同步）
...
- **2026-06-11 新增案例 6**：cron runner 把 191KB stderr 整份倒進 telegram 切 50 則 + 3 個獨立 bug 同時爆（rsync mkdir、gh 帳號、scheduler 沒截斷、v4 早退缺失）+ 完整診斷 SOP（**execution-sop.md SOP-7**：「備份/cron 異常診斷 3 件套」、4 個 tool call 內定位 90% 異常）
...
| 分類 | 檔案 | 條目數 | 觸發情境 |
|------|------|-------|----------|
| hermes-internal | `references/by-category/hermes-internal.md` | **31** | hermes CLI / cron / config 行為 / **驗證閉環失敗 (2026-06-09)** / **OpenClaw 反安裝 (2026-06-08)** / **bash_profile 互動式 return 阻擋 (2026-06-08)** / **SOP 寫設計變數不寫現況快照 (2026-06-09)** / **跨 profile handoff context 累積風險與 kill 接手 (2026-06-10)** / **jobs.json prompt 欄位污染 + context_from 空陣列 + coverage check 雙檔同步 (2026-06-11)** / **hermes-config-backup-daily prompt 殘留 args 修復 + coverage check hidden file 誤判 (2026-06-11)** / **代理重塑後 SOP 範例沒跟著同步 (2026-06-11)** / **keyword 符號選擇 ^ 給 handoff @ 給 skill (2026-06-11)** / **keyword 觸發時主動延伸 user 語意是 Rule 12 違規 (2026-06-11)** |
...
| **Hermes 源碼重啟 SOP (SOP-7)** | `references/sops/hermes-source-restart-sop.md` | **2026-06-11 新建**：改任何 `hermes-agent/*.py` 後必走 7 步重啟 gateway SOP。涵蓋 `sudo systemctl restart` 卡 3 分鐘是正常（systemd `Type=simple` 沒設 `TimeoutStopSec`）、3 條常見錯誤（含 `Refusing to restart from inside gateway process` 怎麼繞）、5 條 If→Then 配套、**一鍵三段驗證命令**（pgrep + systemctl status + journalctl） |
