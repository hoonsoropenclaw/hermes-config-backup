...
async def main():
    browser = await driver.start(
        ...
        headless=True,
        sandbox=False   # 赫米斯 N100 環境必須 False
    )
    tab = await browser.get("https://target-site.com")
    title = tab.title        # property，非 method
    html = await tab.evaluate('document.documentElement.outerHTML')
    print(f'{title}, len={len(str(html))}')
    await browser.stop()     # stop() 放最後，之後 loop 已 closed

asyncio.run(main())
```

**2026-06-21 實測結果（nodriver v0.50.3）**：
- ✅ Chrome 啟動成功（用 Playwright 快取 Chromium，無需另裝）
- ✅ 頁面導航成功（example.com 正常）
- ⚠️ `tab.title` 是 property 不是 method
