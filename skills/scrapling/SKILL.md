...
# 之後網站結構變了？用 adaptive domain 設定
Fetcher.configure(adaptive=True, adaptive_domain='example.com')
f2 = Fetcher()
p2 = f2.get('https://example.com')
products2 = p2.css('.product', adaptive=True)
# Scrapling 會自動學習重新定位！
```
...
```python
# StealthyFetcher - 隱形抓取模式（v0.4.8 實測）
s = StealthyFetcher()
resp = s.get(
    'https://example.com',
    headless=True,           # 無頭模式
...
# 取得多個元素
for item in resp.css('.product'):
    print(item.css('h2::text').get())
```

**⚠️ API 警告（v0.4.8）**：
- `Fetcher.fetch(url)` — **錯誤**：classmethod 不存在，會 `AttributeError`
- `Fetcher.configure()` — **不返回 Fetcher 實例**，返回 `None`；`configure()` 只用來設定全域 parser 參數
- 正確：`f = Fetcher()` 直接實例化後用 `.get(url)` / `.post(url, data=...)`
...
# 隱形（繞過 Cloudflare 等）
s = StealthyFetcher()
resp = s.get('https://example.com', headless=True)

# 動態（JavaScript 渲染）
d = DynamicFetcher()
resp = d.get('https://example.com', wait_for='.content-loaded')
```

**⚠️ 所有 Fetcher 的 `.get(url)` / `.post(url)` 是實例方法，不是 classmethod**
...
def fetch_education_data():
    f = Fetcher()
    p = f.get('https://stats.moe.gov.tw/')
...
- 📖 [stealth-browser-2026.md](references/stealth-browser-2026.md) — 2026 年 stealth browser benchmark（三層指紋原理、抉擇樹更新）；⚠️ nodriver 31/31 零封鎖 claim 未經實測驗證
- 📖 [scraping-decision-tree.md](references/scraping-decision-tree.md) — 多頁面網站爬取整合決策樹（Fetcher / StealthyFetcher / Spider / nodriver / D2 抉擇）
- 📖 [scrapling-api-usage-20260629.md](references/scrapling-api-usage-20260629.md) — v0.4.8 API 實測：`.fetch()` classmethod 不存在、`configure()` 不返回實例的正確用法（2026-06-29）
- 📖 [scrapling-cloudflare-verification-20260629.md](references/scrapling-cloudflare-verification-20260629.md) — Cloudflare benchmark 驗證缺口：StealthyFetcher 對 Cloudflare 保護站點從未實測，待 benchmark action items 填寫

---

## ⚠️ Benchmark 驗證缺口（2026-06-29 確認）

以下宣稱**未經實測驗證**，使用時請自行測試：
