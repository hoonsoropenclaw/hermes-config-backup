...
## Phase 0: Framework Selection (Before Any Building)

**Do this FIRST — choosing the wrong framework costs weeks of rework.**

Use the 5-dimension comparison to match your use case:

| Dimension | LangGraph ★★★★★ | CrewAI ★★★★★ | AutoGen ★★★★ |
|-----------|----------------|--------------|--------------|
| **Production Reliability** | Healthcare, finance, legal, audit trails | Content pipelines, research synthesis | Code generation, Azure shops |
| **Development Speed** | 10-14 engineer-days to prod | **<1 week to working demo** | 3-5 engineer-days |
| **Observability** | LangSmith, every transition logged | Limited | Moderate |
| **Human-in-the-Loop** | ✅ Native mid-workflow checkpoints | ⚠️ Limited | ⚠️ Limited |
| **Cost Predictability** | ✅ Deterministic edges | ⚠️ Hierarchical mode unpredictable | ❌ Conversation loops expensive |

### Decision Tree (answer 3 questions)

**Q1: Do you need human review checkpoints mid-workflow?**
→ Yes → **LangGraph** (pause → collect input → resume, native)
→ No → Go to Q2

**Q2: How fast do you need a working demo?**
→ <1 week → **CrewAI** (20 lines Python, non-engineers can read/modify)
→ Can wait 2+ weeks → Go to Q3

**Q3: Are you in a Microsoft/Azure environment with code generation needs?**
→ Yes → **AutoGen/AG2** (Azure OpenAI native, writer+reviewer agents)
→ No → **LangGraph** (model-agnostic, longest ecosystem support)

### Quick Reference (copy-paste)

```
If: Need audit trail + compliance + resume interrupted workflows
Then: LangGraph — state persisted across sessions, every transition checkpointed

If: Validating whether a use case makes sense before full build
Then: CrewAI — fastest proto, rebuild in LangGraph when hitting production constraints

If: Azure shop + code generation/review + iterative reasoning
Then: AutoGen/AG2 — conversation-driven, strong Microsoft Research backing
```
...
|-----------|---------|-------|
| **OpenAI Agents SDK** | OpenAI-only shops, handoff patterns | Locked to OpenAI models |
| **Google ADK** | Multimodal (Gemini), A2A protocol | April 2025, enterprise Google Cloud |
| **Claude Agent SDK** | Anthropic-first shops | Newer, best with Claude models |

**If→Then**: When the user asks "which framework should I use?" — run the 3-question decision tree before describing any framework.
