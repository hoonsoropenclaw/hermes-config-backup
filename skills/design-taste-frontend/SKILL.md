...
**Before any design work, check if program-project-planner should be triggered.**
...
- Only proceed directly to design work (skip planning) when:
  1. User explicitly writes "直接實作" or "直接做"
  2. Task is a single, simple, well-defined action
  3. Emergency hotfix scenario

**This is a hard rule.** Design without planning often produces beautiful work that solves the wrong problem. When in doubt, trigger planning.
...
### Em-dash audit: the places that slip through (mechanical grep required)

A "just look at the visible text" mental model misses em-dashes that hide in:

1. **`<title>` tag in `<head>`** — invisible on first paint, but still user-visible (browser tab, bookmark, search result).
2. **JS template-literal default values** — `placeholder: "—"`, `text: "—"`, or `${label} — ${subtitle}`. Often a hyphen-shaped fallback used in loading states.
3. **`<meta>` tags** — `description`, `og:title`, `twitter:title`. These are visible in link previews, Google snippets, social shares.
4. **`<link rel="...">` aria-labels** and `alt=""` text. Screen readers, no visual render, but still em-dash and still banned.
5. **CSS `content: "—"` pseudo-elements** — decorative dividers in `::before` / `::after`.
6. **JSON-LD / structured data** — `schema.org` descriptions end up in Google rich results.
7. **`<noscript>` fallback text** — invisible to JS users but visible to crawlers and accessibility tools.
8. **`<template>` elements** — `innerHTML` ready to be cloned into the DOM by JS.

**Mechanical audit (do this before declaring done):**

```bash
# ripgrep all em/en-dashes across the file, with line numbers and context
rg -nP '[\x{2014}\x{2013}]' index.html
# or for an entire project:
rg -nP '[\x{2014}\x{2013}]' src/
```

The audit is a single command. If it returns any lines, fix them all. Do not stop at the first one — em-dashes tend to come in clusters because the LLM that wrote them is in "em-dash mode."

**Allowed replacement characters (in order of preference):**
- `,` + space (most natural for "label — descriptor" patterns)
- `.` (sentence break)
- ` / ` (slash, for the "category — subcategory" feel)
- `·` (middle-dot, rationed — see Section 9.F)
- ` ` (just a space, for adjacent labels in tight layouts)
- `-` (regular hyphen, for compound words / ranges only)

Never replace an em-dash with a double-hyphen `--` or with ` - ` in a way that visually still reads as a long pause. The replacement should be a normal typographic separator, not a poor-em-dash.

### Why the Pre-Flight Check list fails to catch this

The Pre-Flight Check in Section 14 has a line for "ZERO em-dashes (—) anywhere on the page" but the agent reading it tends to interpret "on the page" as "in the visible rendered text." That misses 7 of the 8 locations above. Treat the Pre-Flight Check as a reminder to RUN the grep, not a substitute for running it.
