...
- `n100-hardware-routing-20260618.md` — N100 mini PC hardware profile, image
...
## GPU-Less Routing (No Dedicated GPU: Intel UHD, Old AMD, CPU-Only)

When a user asks for image generation on a machine **without a dedicated GPU** (Intel UHD, integrated AMD, or CPU-only), ComfyUI local is not viable. Route to the right tool:

```bash
# ALWAYS check hardware first — never assume
python3 scripts/hardware_check.py --json
```

| Verdict from hardware_check | Recommended Tool | Status on N100 |
|---------------------------|-----------------|----------------|
| `verdict: cloud` (no GPU) | **MiniMax image-01** (if `MINIMAX_API_KEY` in env) | ✅ Available |
| `verdict: cloud` (no GPU) | **ComfyUI Cloud** (if `COMFY_CLOUD_API_KEY` is set) | ⚠️ Needs setup |
| `verdict: cloud` + no API keys | Warn user: no free GPU path available | ❌ |
| `verdict: ok/marginal` | Local ComfyUI via comfy-cli | ✅ Available |

### Priority: MiniMax image-01 before ComfyUI Cloud

MiniMax's `image-01` model is accessible via the existing `MINIMAX_API_KEY` already configured in `~/.hermes/.env` — **no new API key needed**. Use it for:
- Quick text-to-image without content filter concerns (note: filter still applies)
- Strong prompt adherence; handles complex compositions
- Aspect ratios: 16:9, 4:3, 3:2, 2:3, 3:4, 9:16, 21:9

> ⚠️ **Known limitation**: `image-01` de-escalates body-shape adjectives ("丰腴" → "athletic", "curvy" → "standard"). For character design with specific body types, ComfyUI Cloud + custom model/LoRA is better.

### ComfyUI Cloud as fallback

Requires `COMFY_CLOUD_API_KEY` (from https://platform.comfy.org). See **Path A** in Setup & Onboarding. Use when:
- MiniMax image-01 content filter blocks the use case
- User needs a specific model/LoRA not available via MiniMax
- Production-scale batch generation required

### If `mmx` CLI availability is unclear

MiniMax image-01 is accessible via `mmx-cli` (npx-hosted npm package). On N100:
- **`which mmx`** — ALWAYS returns empty (npm global binaries not in shell $PATH — **not a real unavailability signal**)
- **`npx -y mmx-cli --version`** — returns "mmx 1.x.x" when working (correct probe)
- **`npx -y mmx-cli image generate --prompt "test" --out-dir /tmp --quiet`** — generates an image when fully working

If npx works: use `mmx image generate` directly.
If npx fails: fall back to REST API — `POST https://api.minimax.io/v1/image_generation` with `MINIMAX_API_KEY`.
