...
### `scripts/send_for_signature.py` — HR 文件電子簽署 Bridge

將 `hr-document-automation` 產出的 `.docx` 轉換成 DocuSeal HTML 格式、上傳、發送簽署連結。

```bash
python3.12 ~/.hermes/skills/docuseal/scripts/send_for_signature.py \
    "王小明" "wang@mail.edu.tw" /tmp/offer.docx
```

環境變數：`DOCUSEAL_URL`、`DOCUSEAL_API_KEY`（見上方環境變數章節）。

**功能**：
1. `.docx` → HTML（含 DocuSeal `{{signature:}}` / `{{date:}}` 欄位）
2. 上傳 DocuSeal API 建立 template
3. 發送 email 給簽署人
4. `form.completed` webhook → 通知 HR

**與 hr-document-workflow 的串接**：
- `hr-document-automation` 產出 .docx（錄取通知書 / 聘用合約）
- 本 script 轉換 + 發送 DocuSeal
- 完成後 webhook 觸發赫米斯通知 HR

驗證：
```bash
python3.12 -m py_compile ~/.hermes/skills/docuseal/scripts/send_for_signature.py; echo "exit=$?"
# → exit=0
```
