...
## 從 Vercel KV/Blob 遷移到 Supabase 全棧（從 supabase-migration 吸收，2026-06-20 整合）

> **來源**：本段是從原 `supabase-migration` 技能（devops/supabase-migration）整合過來的。該技能已歸檔、合併入本 skill 作為「Vercel KV/Blob → Supabase 遷移」專節。**2026-06-11 從 school-bulletin 實戰驗證過**。

### 何時選 Supabase 取代 Vercel KV/Blob
...
完整 6 個步驟（env 切換、schema 設計、Storage bucket 設定、RLS、API 層改寫、Vercel 部署驗證）、完整 schema 範本（5 表）、`env-migration-script.py`（自動讀 .env / 寫 Vercel env）、`migration-checklist.md`（13 項驗證）→ 見 `references/vercel-migration-recipe.md`、`references/migration-schema-template.sql`、`references/migration-env-migration-script.py`、`references/migration-migration-checklist.md`。
