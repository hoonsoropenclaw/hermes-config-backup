...
### 🆕 10.16 jobs.json `last_error` 欄位含 API Token → GH013 觸發（Cycle 532, 2026-07-21）

**問題**：`hermes-config-backup-daily` 備份 `cron/jobs.json` 到 GitHub staging repo 時，`last_error` 欄位含 `vcp_82zK...` Vercel token（來自 `sync_md_files.py` 的 `TimeoutExpired: Command '['vercel', '--token', 'vcp_82zK...']' timed out after 120 seconds`），觸發 GH013 secret scanning 拒絕。

**根因**：staging repo 的 `cron/jobs.json` 中 `last_error`/`last_delivery_error` 欄位累積了 script 輸出的錯誤訊息（含 API token）。這些欄位會被 rsync 同步進 staging repo，commit 時被 GitHub secret scanning 攔截。

**修復 SOP**（已驗證 Cycle 532）：
...
print(f'Redacted {redacted} fields')
"
# Step 2: git add + commit + push
cd ~/.hermes/hermes-backup-staging && git add -A && git commit -m "chore: redact API tokens from jobs.json last_error" && git push origin main
```

**Prevention（建議下次實作）**：在 `hermes-backup-v4.sh` 的 rsync `cron/jobs.json` 段加 Python 後處理過濾 `last_error` 中的 API token（不修改 hermes 主體，保持職責分離）。

**If→Then（jobs.json GH013 預防）**：
**If** [備份腳本即將 commit `cron/jobs.json` 到 GitHub staging] **Then** [在 git add 前執行 Python 後處理：掃描並替換所有 `last_error`/`last_delivery_error` 欄位中含 `vcp_/ghp_/sk-/gho_/glpat-` 的 Token，為 `[REDACTED]`]
**Why** [`last_error` 欄位累積 script 錯誤輸出，幾乎一定會含 API token；這不是 jobs.json 的 bug，是錯誤日誌被備份腳本同步進 staging repo；staging repo 含 secret → GitHub secret scanning 100% 攔截]

**驗證命令**：
```bash
# 確認 jobs.json 無 secret patterns
...
### 🆕 10.17 cron-only Script 環境變數 Fallback 設計（2026-06-15 新增）
**問題**：`verify-recovery-chain.sh` 在 cron 環境 100% 失敗（`HERMES_USER_KEY` 拿不到），但手動執行成功。原因是 cron script 沒有 3 層 fallback 設計。

**標準模板**：見 `references/cron-only-script-env-fallback.md`（含 Bash parameter expansion 速查、驗證命令、與 §10.10 的差異）。

**核心原則**：cron 無 TTY + 無登入 shell 環境 + 最小化 PATH。任何 cron script 需要 secrets 必須自己實作 2-3 層 fallback：Layer 1: 環境變數 → Layer 2: `~/.hermes/config/.hermes-<KEYNAME>` 檔 → Layer 3: exit 1。

**驗證命令**：`unset HERMES_USER_KEY && bash ~/.hermes/scripts/verify-recovery-chain.sh` → 預期 EXIT:0。

**If→Then**：**If** 你正在寫一個需要 secrets 的 cron-only script **Then** 先實作 Layer 2 fallback，再部署到 crontab

### 🆕 10.16 Skill script 被意外清空 — 0 bytes 陷阱（2026-06-15 新增）
**問題**：`~/.hermes/skills/<skill>/scripts/<script>.sh` 變成 0 bytes，導致 cron job 失敗但 error message 顯示 "Script not found"（當 symlink 指向目標存在但目標為空）或 bash syntax error。

**根因**：某個赫米斯工具（如 `write_file`、`patch`、`cp` over symlink）意外將 skill script 覆寫為空檔。`stat` 顯示 0 bytes，但 symlink 本身仍存在。

**本 cycle 實際案例**：`verify-recovery-chain.sh` 兩處皆變 0 bytes（`~/.hermes/scripts/` symlink 目標 + `~/.hermes/skills/agent-system-backup/scripts/` 原始檔）。

**修復 SOP**：
1. 先檢查 `/tmp/` 有沒有備份：`find /tmp -name "*verify-recovery*" -exec wc -c {} \;`
2. 從備份 staging 目錄挖：`find ~/.hermes/backups/staging_full_*/full_backups/skills/<skill>/scripts/ -name "<script>.sh" -exec wc -c {} \;`
3. 從技能描述中的 block code 重建（如果 SKILL.md 內有 script 內容）
4. 寫回兩處：jobs.json 指向的 `~/.hermes/scripts/<script>.sh` + skill 原始位置 `~/.hermes/skills/<skill>/scripts/<script>.sh`

**驗證**：
```bash
wc -c ~/.hermes/scripts/<script>.sh  # 確認非 0
wc -c ~/.hermes/skills/<skill>/scripts/<script>.sh  # 確認非 0
bash -n ~/.hermes/scripts/<script>.sh  # 語法檢查
```

**預防**：
- **If** 你正在用 `write_file` 覆寫一個 script **Then** 先 `wc -c` 確認來源有內容
- **If** 備份 script 到新位置 **Then** 用 `python3` 讀寫而不是 `cp`（`cp` over symlink 會 follow 並覆寫目標）
- 重要 script 應該進 Git 版本控制（`~/.hermes/hermes-backup-staging/` 會被備份）

**If→Then**：**If** 發現 cron script 變 0 bytes **Then** 不要只修 `~/.hermes/scripts/` symlink，要確認 skill 原始位置也一併修復，否則下次備份覆寫又會把正確版本蓋成 0 bytes
...
- `references/cron-only-script-env-fallback.md` — **2026-06-15 新增** - cron-only script 環境變數 fallback 設計（與 §10.10 的差異、驗證命令、標準模板）
...
- **`references/coverage-decision-tree.md`** — 「哪些該備、哪些不該備」完整決策樹(從 2026-06-10 根目錄完整盤點提煉)。含 5 段判斷流程 + 各路徑必備/不備/走 Tier 2 加密具體分類表。

### 🆕 v6 — 從 `devops/backup-coverage-check` 吸收(2026-07-11 curator pass)

原 `devops/backup-coverage-check` skill 已歸檔,其內容 100% 包含在本 umbrella skill 內:

| 原 skill 內容 | 對應 umbrella 位置 |
|---|---|
| 「哪裡該備、哪些不該備」完整決策樹 | `references/coverage-decision-tree.md`(已 re-home) |
| 每日驗證腳本(`scripts/coverage_check.sh`)| `scripts/hermes-backup-coverage-check.sh`(已 re-home + 修正 symlink 路徑) |
| 3 層檢查設計 + 通知策略 | 主 SKILL.md §1.5 + §10.5/10.14 + §14.1 + `references/hermes-backup-coverage-pattern.md` |
| 「備份該備什麼」決策 SOP | 主 SKILL.md §「磁碟盤點 SOP」段 + `references/coverage-decision-tree.md` |
| 「改 INVENTORY.md 必同步改 backup script」對照表 | 主 SKILL.md §14.1 + §14.6 |

**為什麼合併**:兩個 skill 都是「v4 同步清單有沒有漏」的設計與維護;合併後維護一份 single source of truth 即可。**不再需要維護獨立的 `backup-coverage-check` skill**。
