...
## music cover — Reference Audio → Cover Version

`mmx music cover` (verified Cycle 489) generates a cover version based on a **reference audio URL**. Distinct from `music generate` which creates from scratch via text prompt.

```bash
mmx music cover <audio_url> [flags]
```

| Flag | Type | Description |
|------|------|-------------|
| `--prompt <text>` | string | Style/genre description for the cover |
| `--vocals <text>` | string | Vocal style override |
| `--out <path>` | string | Save output file |

**When to use `music cover` vs `music generate`**:

| Scenario | Command |
|----------|---------|
| User wants a song in the style of a specific track | `music cover <reference_url>` |
| User wants original music described by mood/genre | `music generate --prompt "..."` |
| User wants lyrics preserved with new style | `music cover` + `--prompt "same lyrics, new arrangement"` |
| User wants purely original from text description | `music generate` |

**Use cases**:
- "Make me a song that sounds like [YouTube link]" → `music cover <url>`
- "Create a lo-fi version of [audio file]" → `music cover <url> --prompt "lo-fi hip hop style"`
- "Generate an upbeat pop song about summer" → `music generate --prompt "An upbeat summer pop song..."`

**Limitation**: requires publicly accessible audio URL (not local file). For local files, upload to a temp host first.
