...
**Layer 2.5 / Layer 3 實作狀態（2026-06-17 更新）**：

| 機制 | 驗證層級 | 狀態 |
|------|----------|------|
| SOP-Agent（arxiv 2501.09316）| Layer 2.5 | ✅ 已實作：`automated-sop-validation` skill + FallbackValidator |
| hermes-dojo（Yonkoo11，MIT）| Layer 3 | ✅ 可部署：state.db 失敗信號驅動的自動技能改進迴圈 |
| CRITIC（ICLR 2024）| Layer 3 | 待工具支援 |
| Reflexion | Layer 3 | 已部分融入 trial-and-error skill |
| Agentic Reward Modeling | Layer 3 | 待可驗證正確性信號 |
| Agent Behavioral Contracts | Layer 3 | 待 runtime enforcement |

**hermes-dojo 安裝評估（2026-06-17）**：GitHub: Yonkoo11/hermes-dojo，MIT license，2026-03 Hackathon 作品。Core loop: `measure → identify weakness → evolve → report`。直接讀 `~/.hermes/state.db` 追蹤失敗信號，自動修補技能，生成 Telegram/CLI 學習曲線報告。部署後赫米斯實現「睡覺時也在變強」。建議優先部署。

**Phase 1.5 cron 健康掃描**：每 2 小時由 metacognitive-learner 執行，檢查所有 jobs `last_status`。發現 error 須交叉驗證 jobs.json 實際值是否已改為 trial-and-error 建議值。
...
|------|---------|
| 更新技能文件 | ✅ 即時生效 |
| 寫入長期記憶 | ✅ 主動寫入 trial-and-error |
| Layer 2.5 自動驗證（產出格式） | ✅ `automated-sop-validation` + FallbackValidator |
| Layer 3 自動改進（失敗信號驅動） | ✅ hermes-dojo 可部署（待安裝） |
| 保證 SOP 100% 遵守 | ❌做不到 |
| 自動發現自己的錯誤 | ⚠️ hermes-dojo 部署後自動 |
