...
## ⚠️ 強制 Preflight 聲明規則（2026-06-14 更新）

> **背景**:school-bulletin（Jun 11-12）部署時,`hermes-deployment-sop` 早就在磁碟上(11.9KB),但部署時**沒人主動對照**,導致 vercel token 遮蔽 bug（Python grep 把 `***` mask 當成 key）、git push 500、接連 v3→v4 兩個版本才對。用戶直接抱怨:「已經那麼多次了,難道沒辦法避免嗎?」
>
> **根因**:文件化 ≠ 系統化。SOP 存在於磁碟,但沒有在正確時機被強制引用。
>
> **Layer 2 約束**:部署命令執行**前**,必須在對話中逐項聲明 Preflight 10 項狀態,否則不准執行部署。

### 部署前強制聲明（觸發即義務）

**If** 要執行 `vercel deploy` / `vercel --prod` / `git push` / `netlify deploy` / `Supabase db push` / **任何對外有副作用的部署命令**  
**Then** 在執行命令**前**,在對話中明確聲明:

```
## Preflight 逐項聲明（部署前必填）
- [x] 1. token 來源: (哪個 env/file,已驗證有效)
- [x] 2. .gitignore: (已確認 secrets 不會進 repo)
- [x] 3. repo 大小: (已執行 du -sh .git/objects)
- [x] 4. 大檔: (已執行 git status --short)
- [x] 5. DDL(如有必要): (已 PG 直連驗證)
- [x] 6. E2E: (已執行 e2e-minimum-checklist,結果:N/N 通過)
- [x] 7. Vercel API payload(含 gitSource.org): (已確認)
- [x] 8. Vercel token 有效: (已執行 curl 驗證)
- [x] 9. push 重試機制: (已就緒 for-loop)
- [x] 10. production URL 類型: (將用主要 alias,不是 hash URL)
```

**If** 任何項目為 `[ ]`（未通過）:停下來,先修復該項,再部署。  
**If** 省略聲明直接執行部署:這是 SOP 偏移,事後必須補聲明並解釋為何省略。
...
- **`references/token-mask-grep-bug.md`** = Python grep 把 `***` mask 當 literal 的 bug + 3 種修復方案（2026-06-14 新增）
...
| 版本 | 日期 | 變更 |
|------|------|------|
| 1.0.1 | 2026-06-14 | 新增「強制 Preflight 聲明規則」段落:部署前必須逐項聲明 10 項狀態,不得省略。新增 2 條 If→Then（部署命令執行前必對照、驗收完成必須附真實命令輸出）。背景:school-bulletin Jun 11-12 部署時 SOP 已存在但沒被對照,導致 vercel token 遮蔽 bug 重複 |
