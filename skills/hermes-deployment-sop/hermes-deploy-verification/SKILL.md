...
## 🌐 Hermes Status Site 部署專節（從 hermes-status-site 吸收，2026-06-20 整合）

> **來源**：本段是從原 `hermes-status-site` 技能（productivity/hermes-status-site）整合過來的。該技能已歸檔、合併入本 skill 作為「status site 部署」專節。狀態網站是一個獨立的部署目標，有自己的 SPA 架構陷阱。
...
- 每個 tab 是**獨立 HTML 檔**在 `tabs/` 資料夾（共 11 個）：overview.html, memory.html, delegation.html, tools.html, skills.html, soul.html, md-files.html, scheduler.html, learning.html, system-info.html, dashboard.html
- 點 tab 時 `index.html` 的 `loadTab('overview')` 函式執行 `fetch(\`tabs/${tabName}.html\`)` → strip DOCTYPE/html/head/body wrapper → 注入 `#tab-content.innerHTML`

**3 個架構陷阱**：
1. **loadTab innerHTML 不執行 `<script>`** — `contentDiv.innerHTML = html` 注入時 HTML5 spec 明確禁止執行新插入的 `<script>`。**修法**：把所有需要執行的 JS 預先算好（用 Python 預算座標寫成 inline SVG 或 inline 數字），或透過 `window.*` 命名空間從 index.html 呼叫。
2. **loadTab 從 `<div id="tab-content">` 取 innerHTML** — 新增內容必須在 `#tab-content` 開合之間！放在外面會被 strip，使用者看不到。
3. **每個 tab 都有獨立的 `<head>` / `<body>` wrapper** — 編輯時不用擔心 head/body 重複。

### 跨 Tab 套用 SOP（給所有 tab 加 X 元素時）

**流程 A**：先 `for f in tabs/*.html; do tail -5 "$f"` 盤點全部 tab 找出結構差異
**流程 B**：11 個 tab 的結尾結構分類（7 個標準 4-space `</div></body></html>`、1 個 0-space、1 個 inline script、1 個無 `</body></html>`、1 個 overview 已有 footer）
**流程 C**：用 batch Python 處理（比逐個 patch 快 10x）
**流程 D**：驗證每個 tab 的新內容真的在 `#tab-content` 內
...
| 5 | headless browser 開 production URL、互動 | 確認 JS + 路由都正常 |

**不可跳過 layer 3-5**：❌ 不可只跑本地 server / 只 `git push` 就回報「完成」。✅ 三者（git push + vercel --prod + production curl）都做才能回報。

### 每日排程流程（更新 skill_usage_stats）

1. **執行統計腳本**：`python3 ~/.hermes/scripts/skill_usage_stats.py`
2. **驗證插入位置**（stats 必須在 tab-skills 內，不是在 tab-content close 之後）：
   ```bash
   cd /home/hoonsoropenclaw/hermes-status-site
   grep -n "hermes-skill-stats\|skills-table" index.html | head -20
   # 正確行為：skills-table 出現 1 次
   # 錯誤行為：skills-table 出現 3+ 次（有多個重複的 stats section）
   ```
3. **Commit + Push**：`git add index.html && git commit -m "update: skill usage stats $(date +%Y-%m-%d)" && git push`
4. **部署到 Vercel**：`vercel --prod --token $VERCEL_API_TOKEN`

### 完整原版內容

11 個 tab 結構速查表、anchor pattern、預防原則、完整 cron 配置、統計腳本說明、預防重複插入機制 → 見 `references/status-site-deployment.md`、`references/status-site-tab-structure-cheatsheet.md`、`references/status-site-path-corrections.md`、`scripts/status-site-skill_usage_stats.py`。
