...
### 類型 R：Production Service 缺乏 Watchdog 導致靜默死亡（2026-06-18 新增）

**症狀**：
- `school-bulletin-system` 已建置（deploy.sh + supabase_migrate.sql，mtime 2026-06-18 00:54）
- `camofox-watchdog` 有 crontab entry，但 `school-bulletin` **沒有對應 watchdog**
- 系統上線後沒人知道它何時 down、何時返回——**靜默死亡**

**觸發情境**：
- 生產服務（如 Vercel deployment、Self-hosted app）建置完成後，沒有第一時間建立對應的 uptime/健康檢查 cron
- `camofox-watchdog` 是標杆（有 crontab entry），但其他新服務沒跟上

**修復**：
```bash
# 1. 建立 watchdog script 到 /tmp/（避免 skill dir 0700 問題）
cat > /tmp/school-bulletin-watchdog.sh << 'EOF'
#!/bin/bash
# watchdog for school-bulletin.vercel.app
set -e

URL="https://school-bulletin.vercel.app"
MAX_RETRIES=3
RETRY_DELAY=10
LOG="/tmp/school-bulletin-watchdog.log"
...
for i in $(seq 1 $MAX_RETRIES); do
  ...
  echo "$(date '+%Y-%m-%d %H:%M:%S') retry $i/$MAX_RETRIES: HTTP $STATUS" >> "$LOG"
  [ $i -lt $MAX_RETRIES ] && sleep $RETRY_DELAY
done

echo "$(date '+%Y-%m-%d %H:%M:%S') FAIL: all retries exhausted" >> "$LOG"
echo "$(date '+%Y-%m-%d %H:%M:%S') FAIL" >> /tmp/school-bulletin-watchdog.err
exit 1
EOF
chmod +x /tmp/school-bulletin-watchdog.sh

# 2. 部署到 crontab
(crontab -l 2>/dev/null | grep -v "school-bulletin-watchdog"; echo "* * * * * /tmp/school-bulletin-watchdog.sh >> /tmp/school-bulletin-watchdog.log 2>&1") | crontab -
...
**Watchdog Script 必要元素**：
1. 健康檢查：`curl -s -o /dev/null -w "%{http_code}" --max-time 15 <URL>`
2. 重試邏輯：3 次重試、每次間隔 10s
3. 雙 log：成功寫 `$SERVICE_watchdog.log`，失敗寫 `$SERVICE_watchdog.err` + 回傳 exit 1
4. **部署到 `/tmp/`**（不是 skill dir），避免 0700 導致 root cron 無法讀取

**If→Then**：**If** 建立任何生產服務（Vercel / self-hosted / Supabase）**Then** 同時或下一次 cycle 建立對應的 watchdog script 並加入 crontab，否則視為「未完成部署」

**預防**：見下方「服務部署後的 Watchdog 必查清單」。
...
### 服務部署後的 Watchdog 必查清單（2026-06-18 新增）

每當赫米斯完成一個**生產服務**的建置，**必須**在同一次或下一次 cycle 執行：

- [ ] 該服務有對應的 `/tmp/<service>-watchdog.sh`（部署到 `/tmp/`，避免 skill dir 0700）
- [ ] crontab 有 entry：`* * * * * /tmp/<service>-watchdog.sh`
- [ ] watchdog 從 `/` 手動執行成功（`bash /tmp/<service>-watchdog.sh && echo "OK"`）
- [ ] watchdog 的 `ERR` 檔案存在於 `/tmp/`（failure log）
- [ ] 若 `deploy.sh` 的 polling timeout 可能不夠 → watchdog 自己有獨立的 polling loop（最多 12×30s = 6m）

**驗證命令**：
```bash
# 確認 crontab 有 watchdog
crontab -l | grep <service>

# 確認 script 存在且可執行
ls -la /tmp/<service>-watchdog.sh
bash /tmp/<service>-watchdog.sh && echo "OK" || echo "FAIL"

# 確認 failure log 位置
ls -la /tmp/<service>-watchdog.err
```
...
| `last_error: Blocked: script path resolves outside the scripts directory` + symlink 指向 skill 目錄 | 類型 Q：jobs.json script 為相對檔名但 Scheduler 只做 path.exists() 檢查、不跟隨 symlink。symlink 本身存在，但 Scheduler 的 `is_within_dir(path, scripts_dir)` 對 symlink 仍走 realpath/resolve 後才比對 | 將 jobs.json `script` 欄位改為**絕對路徑**（`/home/hoonsoropenclaw/.hermes/skills/agent-system-backup/scripts/verify-recovery-chain.sh`），或將 actual script 複製進 `~/.hermes/scripts/` |
...
| hermes-backup-coverage-check exit 1 but manual run exit 0 | 真實 warnings 已修復，之前是 warnings-only exit 0 政策未生效 | stale state，驗證：手動跑 `bash ~/.hermes/scripts/hermes-backup-coverage-check.sh` 確認 exit 0 |
| `last_error: Blocked: script path resolves outside the scripts directory` + script 是 symlink | 類型 Q：Scheduler 的 `is_within_dir()` resolve symlink 後發現 realpath 在 skill 目錄 | jobs.json `script` 改**絕對路徑**到 actual script；驗證：`cd / && bash <absolute_path>` |
| 連續 3 天同類錯誤 | 升級為已知問題，建立新 skill 或 patch 既有 skill |

### 類型 Q：Symlink 導致 Scheduler Path 檢查失敗（2026-06-15 新增）

**症狀**：
```
last_error: Blocked: script path resolves outside the scripts directory (/home/hoonsoropenclaw/.hermes/scripts): 'verify-recovery-chain.sh'
last_status: error
```

**觸發情境**：
- jobs.json 的 `script` 欄位為相對檔名 `verify-recovery-chain.sh`
- 該檔案是 symlink：`~/.hermes/scripts/verify-recovery-chain.sh → ~/.hermes/skills/agent-system-backup/scripts/verify-recovery-chain.sh`
- Scheduler 的 `is_within_dir()` 做 path 比對時，會 resolve symlink → 發現 realpath 在 skill 目錄（`/home/hoonsoropenclaw/.hermes/skills/agent-system-backup/scripts/`），不在 `~/.hermes/scripts/`
- 拒絕執行

**根因**：Scheduler 用 `path.resolve()` 或 `Path.resolve()` 解析 symlink 後再比對，不接受 symlink 指向 scripts 目錄外的實際檔案。

**修復（二選一）**：
1. **直接改 jobs.json script 欄位為絕對路徑**（推薦）：
   ```
   "script": "/home/hoonsoropenclaw/.hermes/skills/agent-system-backup/scripts/verify-recovery-chain.sh"
   ```
2. **把 actual script 複製進 `~/.hermes/scripts/`**（破壞 symlink 一致性，不推薦）
...
for j in d['jobs']:
    if 'recovery' in j.get('name','').lower():
        print('name:', j['name'])
...
# 確認 script 存在且可執行
ls -la /home/hoonsoropenclaw/.hermes/skills/agent-system-backup/scripts/verify-recovery-chain.sh

# 手動執行測試（從 / 模擬 cron 環境）
cd / && bash /home/hoonsoropenclaw/.hermes/skills/agent-system-backup/scripts/verify-recovery-chain.sh
```

**If→Then 規則**：
- **If** jobs.json 的 `script` 是相對檔名且該檔案是 symlink **Then** 將 script 欄位改為**絕對路徑**，不要留相對檔名
- **If** cron job 的 `last_error` 包含 `script path resolves outside` **Then** 立即檢查該 script 是否為 symlink，schema fix = jobs.json 改 absolute path

**預防**：新建 cron job 時，script 欄位一律用**絕對路徑**，避免 Scheduler symlink 檢查攔截。
...
- `references/school-bulletin-watchdog-d3-exit-20260620.md` — cycle 060 設計了 watchdog 但從未部署至 crontab，cycle 061 D3 exit 並驗證
