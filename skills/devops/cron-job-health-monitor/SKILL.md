...
| 錯誤含 `Script timed out after 600s` + jobs.json timeout_seconds 已設 600 | 類型 N（v3 表現）：Scheduler `_DEFAULT_SCRIPT_TIMEOUT` 仍可能是寫死值，即使 jobs.json 設 600s 仍不夠（rclone sync ~750MB 需要 8-10 分鐘）。見 `references/hermes-backup-timeout.md`。三層都要設定：`HERMES_CRON_SCRIPT_TIMEOUT=1800` in `.env` + `cron.script_timeout_seconds: 1800` in `config.yaml` + 重啟 gateway |
| 錯誤含 `Script timed out after 600s` + jobs.json timeout_seconds 已設 600 | 類型 N（v3 表現）：Scheduler `_DEFAULT_SCRIPT_TIMEOUT` 仍是寫死值，jobs.json `timeout_seconds: 600` 有被讀取但仍不夠（rclone sync ~750MB 需要 8-10 分鐘，600s 無 buffer）。見 `references/hermes-backup-timeout.md`。三層都要設定：`HERMES_CRON_SCRIPT_TIMEOUT=1800` in `.env` + `cron.script_timeout_seconds: 1800` in `config.yaml` + `hermes cron edit --timeout 1800` + 重啟 gateway |
