...
## Token 與 API Key 設定優先順序（從 hermes-config-checklist 吸收，2026-06-20 整合）

> **來源**：本段是從原 `hermes-config-checklist` 技能的內容整合過來的。該技能已歸檔、合併入本 skill 作為「設定優先順序」專節。
...
### Skills Scan 與 Tool Use Enforcement 的差別（重要）
...
### 第七步：替代 Token 與多帳號工作流

> **場景**：使用者有多個 GitHub / Vercel 帳號，備用帳號的 token 不走 `gh auth login`，需要本地保存並跨 session 重複使用。
...
完整「對話統整模式」、驗證 SOP、`.env` 改動範本等細節保留在 `references/setup-checklist.md`。

### 吸收自原 hermes-config-checklist 的 references 檔案

- `references/checklist-api-verification-results.md` — 各 API 驗證結果彙整
