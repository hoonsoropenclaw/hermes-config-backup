---
name: kanban
description: Hermes Kanban — multi-agent work-queue class covering both the orchestrator view (route work, decompose, never execute) and the worker view (orient → work → heartbeat → block/complete). TRIGGER on Hermes Kanban dispatch (worker spawned with `--skills kanban`), orchestrator-profile routing questions, or task-graph decomposition requests. Both halves of the lifecycle are auto-injected via `KANBAN_GUIDANCE`; this skill is the deeper playbook for each role.
version: 4.0.0
platforms: [linux, macos, windows]
environments: [kanban]
metadata:
  hermes:
    tags: [kanban, multi-agent, orchestration, worker, routing, handoff]
    related_skills: [orchestrator-worker-architecture, agent-identity-management, general-workflow]
---

## Umbrella relationship (2026-09 curator pass)

Kanban is the multi-agent work-queue subsection of agent orchestration. This class subsumes the prior `kanban-orchestrator` and `kanban-worker` siblings — they are now sections of this single class-level skill. Decomposition/routing lives in "Orchestrator view"; lifecycle/handoff/pitfalls lives in "Worker view." Dated ticket or board incidents belong in `trial-and-error`, not as siblings here.

The **core worker lifecycle** (6 steps: orient → work → heartbeat → block/complete) is auto-injected into every kanban process via the `KANBAN_GUIDANCE` system-prompt block. This skill is the deeper playbook for each role; check the KANBAN_GUIDANCE block first before re-reading the corresponding section below.

---

## Which role am I playing?

Hermes Kanban has two distinct roles with different tool expectations:

| Role | Trigger | Tools you have | Tools you should NOT use |
|------|---------|----------------|---------------------------|
| **Orchestrator** | Spawned as a planner/router profile, or as the user-facing session that decomposes work | `kanban_create`, `kanban_link`, `kanban_list`, `kanban_show`, `kanban_complete`, `kanban_block`, `kanban_comment`, profile discovery via `hermes profile list` | terminal/file/code/web for execution; `delegate_task` for what should be a kanban card |
| **Worker** | Spawned with `--skills kanban` by the dispatcher, or any process whose task body lives in a kanban row | `kanban_show`, `kanban_complete`, `kanban_block`, `kanban_comment`, `kanban_create` (for follow-ups assigned to OTHER specialists) | `delegate_task` as a substitute for `kanban_create`; `clarify` (you are headless); modifying files outside `$HERMES_KANBAN_WORKSPACE` |

If you don't know which role you're in: check whether the system prompt includes `KANBAN_GUIDANCE` with worker lifecycle steps → worker. Otherwise orchestrator.

---

# Part 1: Orchestrator view (route, don't execute)

> Loaded when you're an orchestrator profile whose whole job is routing work to specialists.
...
# Part 2: Worker view (orient → work → heartbeat → block/complete)

> You're seeing this section because the Hermes Kanban dispatcher spawned you as a worker with `--skills kanban` — it's loaded automatically for every dispatched worker. The **lifecycle** (6 steps: orient → work → heartbeat → block/complete) also lives in the `KANBAN_GUIDANCE` block that's auto-injected into your system prompt. This section is the deeper detail: good handoff shapes, retry diagnostics, edge cases.
...
kanban_complete(
    ...
    metadata={
        "sources_read": 12,
        "recommendation": "vLLM",
        "benchmarks": {"vllm": 1.0, "sglm": 0.87, "trtllm": 0.72},
    },
)
```
...
- `references/orchestrator-view.md` — verbatim previous `kanban-orchestrator` SKILL.md content (preserved for archival traceability)
- `references/worker-view.md` — verbatim previous `kanban-worker` SKILL.md content (preserved for archival traceability)