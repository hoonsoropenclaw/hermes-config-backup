...
#   hermes-backup-v4.sh --brief          # 簡化輸出（cron Telegram 用、err() 仍全顯示）
#
# 排程建議（cron）：
#   每天 02:00 跑 tier1（增量 git push）  建議加 --brief
...
log()  { $BRIEF || echo -e "${CYN}[$(date +%H:%M:%S)]${NC} $*"; }
ok()   { $BRIEF || echo -e "${GRN}[$(date +%H:%M:%S)] ✓${NC} $*"; }
warn() { echo -e "${YLW}[$(date +%H:%M:%S)] ⚠${NC} $*"; }   # brief 仍顯示 warning
err()  { echo -e "${RED}[$(date +%H:%M:%S)] ✗${NC} $*" >&2; } # brief 仍顯示 error（重要診斷）

run_or_dry() {
  if $DRY_RUN; then
    echo -e "${YLW}[DRY]${NC} $*"
  elif $BRIEF; then
    # brief 模式：把所有底層指令的輸出丟掉，只保留 err/warn 跟最終 summary
...
tier1_github() {
  ...
  if $DRY_RUN; then
    ...
    # brief 模式仍要捕獲 push output（驗證 GH013/GH001 錯誤用），但 tail -10 跳過
    local push_output
    push_output=$(git push origin main 2>&1) || true
    $BRIEF || echo "$push_output" | tail -10
...
tier2_drive() {
  ...
  if $DRY_RUN; then
    ...
    # brief 模式：把 secrets-encrypt 自己的輸出丟掉，只保留最終 summary
    "$encrypt_script" "${args[@]}" > /dev/null 2>&1 || return 1
  else
    "$encrypt_script" "${args[@]}"
  fi
  ...
  if ! $DRY_RUN; then
    if $BRIEF; then
      # brief 模式：包住子函式輸出（passphrase-recovery 跟 restore-readme 上傳的 rclone 進度）
      ( backup_passphrase_recovery && upload_drive_restore_readme ) > /dev/null 2>&1 || return 1
    else
...
main() {
  ...
  if $DO_TIER1; then
    if ! tier1_github; then
      ...
      echo "❌ v4 backup FAIL — Tier 1 error（看 ~/.hermes/logs/$(date +%Y%m%d)_hermes-backup-v4.log）"
      return 1
    fi
    $BRIEF || echo ""
  fi
  ...
  if $BRIEF; then
    ...
    if [[ $exit_code -eq 0 ]]; then
      # 一行總結、Telegram 友善、無 ANSI 色碼（直接 deliver 到 telegram）
      echo "✅ v4 backup OK — $(date +%H:%M)$([ "$DO_TIER1" = true ] && echo ' · Tier1' || true)$([ "$DO_TIER2" = true ] && echo ' · Tier2' || true)$([ "$DO_UPLOAD_T2" = true ] && echo '·upload' || true) · ${elapsed}s"
    else
      echo "❌ v4 backup FAIL — $(date +%H:%M) · Tier2 error · ${elapsed}s（看 ~/.hermes/logs/$(date +%Y%m%d)_hermes-backup-v4.log）"
    fi
  else
    echo ""
    if [[ $exit_code -eq 0 ]]; then
