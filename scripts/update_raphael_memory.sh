...
# 找到「核心職責」部分結束的位置（在「## 重要設定日期」之前）
# 我們要在「核心職責」和「重要設定日期」之間插入新內容

# 先找到「## 核心職責」的結束位置
core_duties_end=$(echo "$content" | grep -n '## 重要設定日期' | head -1 | cut -d: -f1)

if [ -z "$core_duties_end" ]; then
    echo "錯誤：找不到「重要設定日期」標題"
    exit 1
fi
...
# 分割檔案
head_content=$(echo "$content" | head -n $((core_duties_end - 1)))
tail_content=$(echo "$content" | tail -n +$core_duties_end)
