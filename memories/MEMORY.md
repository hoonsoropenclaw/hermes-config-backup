...
- 2026-06-11: **notify_on_complete 通知延遲 10-14 分鐘是常態**——v2 架構派 4 worker + 1 summarizer,實際 11 分鐘跑完但通知 10-14 分鐘才送達。L3 教訓:「Hermes 通知是『最終確認』不是『即時 polling』,用 `ls <output_dir>` 撈實際產出比等通知更可靠」。**If** 派 background process **Then** 主動用 ls 監聽、不依賴 notify
- 2026-06-11: **handoff chain 收尾必跑「PRD 對照驗收」4 步（慘痛教訓）**——school-bulletin 跑 4 棒 handoff、engineering-lead 產 22 檔 + build pass、我以為完成,使用者一測才發現 9 個 Must 只實作 5 個(55%)、密碼修改完全沒做、推播/RBAC 完全沒寫。L3 教訓: 「handoff 鏈尾必跑 4 步驗收」(完整 SOP 在 `skills/trial-and-error/references/sops/handoff-chain-acceptance-sop.md`)。**If** 跑完任何 ≥ 2 棒 chain **Then** 4 步必跑、缺一不收;**If** Must 是 🟡 或 ❌ **Then** 不交付、必補或主動告知落差
- 2026-06-11: **handoff chain timeout 升 1200s + 5min monitor 腳本**——600s 太短、sub-agent 升套件+整合+寫大檔會超時。**升到 timeout=1200s(20 分鐘)** + `~/.local/bin/handoff-monitor <slug> <wrapper>` 監控。派遣標準: `terminal(background=True, timeout=1200, notify_on_complete=True)` + background 跑 monitor。完整 SOP: `skills/trial-and-error/references/sops/handoff-chain-timeout-sop.md`
- 2026-06-11: **Vercel 手動 deploy 腳本**——GitHub 整合有時失效(`link: None`)、push 60s 沒新 deploy。解法 `~/.local/bin/vercel-deploy [slug] [branch]`,token 走 env var 避免內容過濾器。**If** push 完 60s 沒新 deploy **Then** 直接跑
