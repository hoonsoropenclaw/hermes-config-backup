...
**不適用範圍**：
- `~/.hermes/` 內部配置 → 用 `hermes-config-layout` skill（**不要**用本 skill 整理 `~/.hermes/` 內部）
- `~/.openclaw/` 系統資料 → 不手動管
- 使用者專案內部結構（每個專案自己的 AGENTS.md / README 才是權威）

> **2026-06-10 重要邊界確認**：當使用者說「Y:\\.hermes 整理」「~/.hermes 看起來很亂」時，**不要**套用本 skill 的「三分層」決策（permanent / temporary / tools）——`~/.hermes/` 是 hermes agent 自己的家，**有完全不同的內部慣例**（見 `hermes-config-layout` skill「磁碟盤點 SOP」段）。本 skill 的磁碟盤點 SOP 仍適用於「Y:\\ 根目錄」「~/permanent-projects/」等其他目錄。
...
**腳本設計符合「高效率耗盡」使用者偏好**（2026-06-10）：動手前印出所有資訊（label 必填、來源/目標/備份位置）、SHA256 雙保險驗證、rollback 機制、還原指令在輸出末段。**未來任何單檔搬遷**都該優先用這個腳本而不是手動 4 步。
...
- ❌ **「先備份後搬動」流程沒走完**：使用者 INTJ 對驗證要求嚴格，少任何一步都會被審查時抓出來

### 「零風險清理」標準 SOP（2026-06-10 完整示範，給未來磁碟盤點任務用）

赫米斯使用者的「先備份後再搬動、需要檢查驗證無錯誤、一步一步確認」流程已成標準。**任何根目錄 / 工作區的檔案清理 / 搬遷必走這 5 階段**：

#### Phase 1：盤點（不動任何東西）
- `find . -maxdepth 1 -type f` + `du -sh <每個檔案>` 列出根目錄所有檔案
- 每個檔案分類：**系統設計就在根目錄（不能動）** / **應該在子目錄（可搬）** / **應該清掉（可刪）** / **孤兒（需使用者決策）**
- 給使用者完整評估報告（每個檔案「建議位置」「分類」「風險」「為什麼」）

#### Phase 2：先備份
- SHA256 fingerprint：`sha256sum <targets> | tee /tmp/<task>-<date>.sha256`
- `/tmp` 雙保險副本：`cp -p <targets> /tmp/<task>-backup-<date>/`
- 永久備份（如果可逆價值高）：`mkdir -p archive/ && cp -p <target> archive/<name>.<version>-<date>`
- 驗證備份的 SHA256 跟現況一致

#### Phase 3：分階段動手（**每階段獨立驗證**）
- **每個動作都單獨執行**，不要一次跳好幾步
- 每步完跑：
  1. 預期值 vs 實測值對照（SHA256、檔案存在性、內容前 N 行）
  2. 該動作的「動完後世界應該長什麼樣」清單核對
- **失敗要可還原**：每步都設計成「後悔隨時可 undo」、驗證命令留痕跡
- 給使用者「動手前最後一次報告」再執行

#### Phase 4：完整驗證
- 跑所有目標檔案的最終 SHA256
- 跑「這個變更沒影響外部依賴」的 smoke test（設定檔讀取、CLI 啟動、關聯 service 跑得起來）
- `/tmp` 雙保險備份還在、archive/ 永久備份歸檔完整

#### Phase 5：還原指令留底
- 給使用者一鍵還原指令（archive 來源 / `/tmp` 來源 / git revert 等）
- 寫進 `archive/README.md` 或同等位置

**If** 使用者說「先備份後再搬動」「需要檢查驗證無錯誤」「一步一步確認」**Then** 走這 5 階段、不要跳
**If** 任何 Phase 卡住（SHA256 對不上、驗證失敗、外部依賴壞掉）**Then** 立即停、回報、絕不繼續往前
**If** 評估階段發現「某個檔案有上游 hermes 程式碼 hardcode 路徑」**Then** 必須先 grep 確認、不能動，否則 hermes 重啟時會自動重建
...
| `hermes-config-layout` | 那管 `~/.hermes/` 內部配置；本 skill 管 `~/*` 使用者工作區。**不要混**——盤點 `~/.hermes/` 用 hermes-config-layout 的磁碟盤點 SOP、盤點 `~/` 用本 skill |
...
### `~/.hermes/` 內部盤點 vs 其他目錄（**重要邊界**）

| 目標目錄 | 用哪個 skill | 為什麼 |
|---------|-------------|--------|
| `~/.hermes/` 根目錄 | **`hermes-config-layout`** 「磁碟盤點 SOP」 | hermes 上游有 hardcode 慣例 + `USER_OWNED_EXCLUDE` 白名單，不是「三分層」邏輯 |
| `~/.hermes/memories/` 內 7 個重要檔案 | **直接 grep 程式碼**確認實際路徑（不要再被 AGENTS.md 誤導） | 例如 `SOUL.md` 必須在根目錄不是 `memories/` |
| `~/.hermes/scripts/` 內的腳本 | 本 skill 的「搬移 SOP」 | 跟 `~/scripts/` 同性質（可執行腳本）|
| `~/` 根目錄、Y 槽專案目錄 | **本 skill**「三分層 + 磁碟盤點 SOP」 | 使用者工作區、適用三分層架構 |
| `~/permanent-projects/` `~/temporary-projects/` `~/tools/` | **本 skill** | 已知分類目錄、搬移 SOP 適用 |

**If** 使用者說「Y:\\.hermes 整理」「~/.hermes/ 亂」 **Then** 載入 `hermes-config-layout` 跑「磁碟盤點 SOP」,**不要**用本 skill 三分層
...
| 版本 | 日期 | 變更 |
|------|------|------|
| 1.2.0 | 2026-06-10 | 加強「`~/.hermes/` 內部盤點」邊界：明示「不適用範圍」段加 2026-06-10 邊界確認（**不要**套用三分層到 `~/.hermes/`），「與其他 skill 的關係」段加「`~/.hermes/` 內部 vs 其他目錄」對照表（5 種目標用哪個 skill 一目了然）。**新增 `scripts/atomic-move-with-verify.sh`**：把「備份/SHA256/mv/驗證/rollback」做成一個指令，符合「高效率耗盡」使用者偏好（動手前評估 + 每步備份 + 失敗可還原）。**未來單檔搬遷**應優先用這個腳本 |
