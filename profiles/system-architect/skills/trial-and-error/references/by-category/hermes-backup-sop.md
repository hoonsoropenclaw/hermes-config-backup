...
- 5 個自建 skill：metacognitive-learner、hermes-agent、hermes-tier-router、trial-and-error、alt-token-secrets-layout
  - 註：`persistent-subagent` 與 `~/.hermes/agents/` 已於 2026-06-09 移除（舊「派遣 subagent 讀身份檔」方案，改採 `hermes profile + tmux` 方式建立常駐代理）
