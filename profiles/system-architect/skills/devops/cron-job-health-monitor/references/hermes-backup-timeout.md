...
## 2026-06-10 新案例（Type N2）：Script 檔案根本不存在

**問題**：`hermes-config-backup-daily` cron job timeout，錯誤訊息：
```
Script timed out after 600s: /home/hoonsoropenclaw/.hermes/scripts/backup_hermes_v3.sh
```
jobs.json 中 `script: backup_hermes_v3.sh`，但 `ls ~/.hermes/scripts/backup_hermes_v3.sh` → No such file。

**根因**：trial-and-error 條目記錄了「jobs.json 改 script 欄位」，但**從未驗證 script 檔案是否真的存在於 `~/.hermes/scripts/`**。V3-BACKUP-STATUS.md 記載 v3 實驗因 Drive throttle 未完成、從未接 cron。

**交叉驗證失效**：「文件說了要改，但根本沒改」。

**修復**（二選一）：
1. **快速方案**：將 jobs.json 改回 `script: backup_hermes.sh`（v2 確認存在：`ls -la ~/.hermes/scripts/backup_hermes.sh` → 19976 bytes）
2. **完整方案**：從 staging 取回 v3 腳本、部署到正確位置

**驗證命令**：
```bash
ls -la ~/.hermes/scripts/backup_hermes_v3.sh   # 確認存在（目前不存在）
ls -la ~/.hermes/scripts/backup_hermes.sh      # fallback v2（存在，19976 bytes）
```

**If→Then**：**If** cron job error 且 trial-and-error 已有修復記錄 **Then** 必須同步驗證：script 檔案 `mtime` 在 trial-and-error 記錄的「修復日期」之後。**文件改了 ≠ 檔案部署了。**