...
### Container/VM Environment: "No usable sandbox" Error

When running on Linux inside a container, VM, or CI environment (including Vercel, Docker, N100 etc.), you may see:
```
Chrome exited before providing DevTools URL
No usable sandbox! If you are running on Ubuntu 23.10+...
```

**Fix**: Always pass `--args "--no-sandbox"` to the `open` command:
```bash
agent-browser open <url> --args "--no-sandbox"
```
This is the standard pattern for ALL headless Chrome in unprivileged/container environments. Treat it as the default when running on N100, Vercel, or any Linux VM.
