...
### SOUL.md 跟其他重要檔交叉比對 6 個衝突修正（2026-06-10）

**症狀**：SOUL.md 8957B 寫太久（6 個過時/重複/衝突段）、跟其他重要檔（AGENTS/IDENTITY/USER/HEARTBEAT/MEMORY/TOOLS）內容重疊、互相矛盾

**根因**：SOUL.md 是「人格宣言」性質、但過去被當成「什麼都能塞」的垃圾場（Python 開發守則、env 變數、cache 結構、資料夾結構、Session 延續性三層備援都寫在裡面）。其他重要檔成熟後忘了同步、形成「過時教條」。

**解法**（6 個衝突修正）：
1. **「耗盡配額」語意不清**：原寫「全力消耗」「耗盡為榮耀」沒「高效率」關鍵字。改為「**高效率耗盡**」+ 3 個可驗證指標（動手前先評估、每步備份+驗證鏈、失敗要可還原）+ 4 個反例（沒評估就 rm -rf、一次跳好幾步、改了設定不知道怎麼 revert、浪費 token retry）
2. **「第一次就做對」vs「主動學習循環」**：前者要求「一次做對」、後者暗示「迭代改進」會矛盾。統一為「品質內化」機制（過去失敗被內化 = 未來第一次就做對）
3. **「回應指示燈」過時**：🟢/🔴 泛工作流 emoji 跟 USER.md「直接精確、效率優先」風格不符。整段刪
4. **Python 開發守則跟 TOOLS.md 重複**：搬去 `memories/TOOLS.md`「🐍 Python 開發與套件安裝守則」段、加搬遷日期註記
5. **「資料夾結構」段過時**：只列 5 個目錄、實際有 30+ 個。整段刪（AGENTS.md 已有新版速查表）
6. **「Session 延續性...三層備援」跟 USER.md「預設不寫」衝突**：整段刪（HEARTBEAT.md 已有新版「兩階段記憶搜尋規則」）
7. **Vibe 段加 USER.md 呼應**：原寫「Be the assistant you'd actually want to talk to. Concise when needed」不夠具體。加「跟 USER.md 溝通風格偏好一致：直接精確、效率優先、結構化、不遺漏」

**預防**：
- SOUL.md 只留「人格宣言 + 行為原則」、不超過 120 行
- 每週掃一次「重疊/過時」段
- 重要檔「職責分離」原則：SOUL=人格宣言、TOOLS=本機環境、MEMORY=抽象教訓、USER=使用者偏好、IDENTITY=身份卡、AGENTS=工作區、HEARTBEAT=任務清單
- 改完必跑 hermes 模擬讀取（python `load_soul_md()`）確認 persona 真的會被載入

**If→Then**：
- **If** SOUL.md 跟其他重要檔衝突 **Then** 必逐檔交叉比對 + 改檔對照表
- **If** SOUL.md 超過 120 行 **Then** 必有重複或過時段、必掃一次
- **If** 重要檔內容重疊 **Then** 必按「職責分離」原則分層 (SOUL=人格,TOOLS=環境,MEMORY=教訓,USER=偏好)

**相關條目**：
- [[hermes-internal#SOUL.md 永遠在 HERMES_HOME 根目錄、不在 memories/]]
- [[hermes-internal#AGENTS.md 7 個重要檔必建「實際路徑速查表」(讀哪/改哪/備份哪)]]
