...
| 代理類型 | 範例專屬 skill |
|----------|----------------|
| 市場策略 | `market-research`、anthropic-competitive-*、anthropic-customer-research、research、blogwatcher、polymarket |
...
6. **白名單設計時「拼錯的 skill 名」是常見陷阱(2026-06-10 驗證)** — 用記憶中的 skill 名稱設計白名單時,46 個有 2-3 個是「我以為有但磁碟沒」或「磁碟有但我拼錯」。**修法**:設計白名單時**先 `ls ~/.hermes/profiles/<name>/skills/` 對照一次**、用磁碟實際有的名字填白名單,再計算「白名單 ∩ 磁碟 = 要保留」的真實交集。
7. **精瘦後的 skill 數量可能比預期多 10-20 個(2026-06-10 驗證)** — 預期 75-85 個但算出 99 個,差 14 個是「真實必要」。白名單寧可寬鬆 10-15 個也不要死守上限 — 嚴守上限會誤刪必要 skill(架構師需要 API/SQL/雲端/SPARC/資安/視覺化/frontend 知識,涵蓋面比 consumer/product-planner 廣)
8. **白名單設計要分類「共同必留 + 角色專屬」兩段(2026-06-10 驗證)** — 混合在一起設計容易漏;分兩段後,「共同必留」30 個是任何代理共享的,「角色專屬」70 個是這個代理的特化,可以分頭評估
